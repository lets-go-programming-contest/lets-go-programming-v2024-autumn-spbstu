...
import (
	...
	myWifi "github.com/kirill.romanchuk/task-7/internal/wifi"
...
var testTable = []rowTestSysInfo{
	...
	{
		addrs:       []string{"00:11:22:33:44:55"},
		errExpected: nil,
	},
	{
		addrs:       nil,
...
func TestGetName(t *testing.T) {
	...
	for i, row := range testTable {
		...
		require.Equal(t, getInterfaceNames(row.addrs), actualNames, "row: %d, expected addrs: %s, actual addrs: %s", i, getInterfaceNames(row.addrs), actualNames)
	}
}
...
func getInterfaceNames(addrs []string) []string {
	interfaces := mockIfaces(addrs)
	var names []string
